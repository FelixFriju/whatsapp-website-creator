...
/**
 * website1 — shared building blocks
 * Small, dependency-light utilities used across the template's pages.
 */
...
/* ------------------------------------------------------------------ */
/* SmartImage — graceful fallback when the platform omits an image    */
/* ------------------------------------------------------------------ */

/**
 * Renders an image when a URL is supplied; otherwise an elegant neutral
 * block with a monogram/initial so layouts never break. Pure CSS fallback,
 * no AI generation.
 */
export function SmartImage({
  ...
  src?: string;
  alt: string;
  /** Letters shown in the fallback block */
  monogram?: string;
  className?: string;
  /** Wrapper class that sets aspect-ratio (e.g. s.ratioHero) */
  ratioClass?: string;
  loading?: "lazy" | "eager";
}) {
  ...
  return (
    <div className={`${s.imgWrap} ${ratioClass ?? ""} ${className ?? ""}`}>
      {showImg ? (
        <img
          src={src}
          alt={alt}
        ...
        <div className={s.imgFallback} role="img" aria-label={alt}>
          <span className={s.imgFallbackMark}>{monogram ?? "•"}</span>
        </div>
      )}
    </div>
  );
...
/* ------------------------------------------------------------------ */
/* Reveal — subtle scroll-in animation (IntersectionObserver)         */
/* ------------------------------------------------------------------ */
...
export function Reveal({
  ...
  return (
    <Tag
      ...
      ref={ref as any}
      className={`${s.reveal} ${shown ? s.revealShown : ""} ${className ?? ""}`}
      style={style}
    >
      {children}
    </Tag>
...
/* ------------------------------------------------------------------ */
/* CtaBand — closing call-to-action used on Home and About            */
/* ------------------------------------------------------------------ */

export function CtaBand({ data, basePath }: { data: WebsiteData; basePath: string }) {
  return (
    <section className={s.ctaBand} aria-label="Call to action">
      <div className={s.container}>
        <Reveal className={s.ctaBandInner}>
          <div className={s.ctaBandCopy}>
            <p className={s.eyebrow}>{data.businessType}</p>
            <h2 className={s.ctaBandTitle}>{data.content.ctaText}</h2>
            <p className={s.ctaBandSub}>
              Speak directly with {data.companyName} — by telephone, email, or WhatsApp.
            </p>
          </div>
          <div className={s.ctaBandActions}>
            <a className={`${s.btn} ${s.btnLight}`} href={pageHref(basePath, "contact")}>
              Contact Us
            </a>
            <a
